...
function generateVideoPrompt(sb, style, videoRatio) {
  // Omni segment text is the authored shot, not another source to merge with stale fields.
  if (sb.creation_mode === 'universal' && String(sb.universal_segment_text || '').trim()) {
    return String(sb.universal_segment_text).trim();
  }
  const parts = [];
  if (sb.description) parts.push('剧情依据（人物、动作和事件顺序以此为准）：' + sb.description);
  // 场景与标题
  ...
  // 动作与对白（核心叙事）
  if (!sb.description && sb.action) parts.push('动作：' + sb.action);
  if (sb.dialogue) parts.push('对话：' + sb.dialogue);
  if (sb.narration) parts.push('解说旁白：' + sb.narration);
  if (!sb.description && sb.result) parts.push('结果：' + sb.result);
  // 镜头与运镜
...
function deriveStoryboardFieldsFromAi(sb, style, videoRatio, opts = {}) {
  ...
  let videoPrompt = generateVideoPrompt(sbWithAngles, style, videoRatio);
  ...
  if (universalOmni && universalSegmentText) videoPrompt = universalSegmentText;
  return {
    shotNumber,
    title,
    shotType,

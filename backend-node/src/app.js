...
function createApp() {
  ...
  const app = express();
  // Keep API responses, including local model credentials, inaccessible to external origins.
  app.use((req, res, next) => {
    if (req.headers.origin) {
      try {
        const origin = new URL(req.headers.origin);
        if (!['localhost', '127.0.0.1'].includes(origin.hostname)) return res.status(403).json({ error: '仅允许本机工作台提交请求' });
      } catch (_) { return res.status(403).json({ error: '请求来源无效' }); }
    }
    next();
  });
  ...
  app.use('/api/v1/studio', studio.routes(db, config, log));
  app.use('/api/v1', studio.guardLegacy(db), setupRouter(config, db, log));
  require('./services/studioService').recover(db, config, log);

...
const USERDATA_DIR = process.env.STUDIO_USER_DATA || path.join(app.getPath('appData'), 'xumu-studio');
...
;(function migrateOldUserData() {
  ...
  if (!process.env.STUDIO_USER_DATA && USERDATA_DIR.endsWith('localminidrama-desktop') && fs.existsSync(oldPath) && !fs.existsSync(USERDATA_DIR)) {
    try {
...
function createWindow(port) {
  ...
  win.webContents.session.setPermissionRequestHandler((contents, permission, callback, details) => {
    callback(permission === 'media' && contents === win.webContents &&
      contents.getURL().startsWith(localOrigin + '/') && !(details.mediaTypes || []).includes('video'));
  });

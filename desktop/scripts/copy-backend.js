...
const dirsToCopy = ['src', 'configs', 'scripts', 'migrations', 'examples'];
...
if (!path.resolve(dest).startsWith(path.resolve(repoRoot, 'desktop') + path.sep)) throw new Error('Invalid backend copy target');
fs.mkdirSync(dest, { recursive: true });
// Keep the directory itself: an exiting Electron subprocess may still hold its cwd.

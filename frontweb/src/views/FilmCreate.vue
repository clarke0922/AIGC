          ...
          <div :id="'sb-' + sb.id" class="storyboard-row">
            ...
            <div class="sb-panel sb-video">
              ...
              <div class="sb-video-params-bar">
                <span class="sb-video-prompt-text sb-video-prompt-text--preview">{{ sb.video_prompt || '暂无视频提示词（在「视频配置」保存后自动生成）' }}<small v-if="isSbUniversalMode(sb.id)" style="display:block;margin-top:8px">全能模式以片段描述为准发送视频请求。修改描述后，可在视频配置中重新生成提示词；旧提示词不会覆盖片段描述。</small></span>
...
const {
  ...
  charRoleLabel, onGenerateCharacters: onGenerateCharactersRaw, openAddCharacter, editCharacter,
